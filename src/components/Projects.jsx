...
const ProjectCard = ({ project }) => {
  return (
    <div className="relative w-full h-80 rounded-2xl cursor-pointer group overflow-hidden shadow-md">
      {/* Card front - visible initially */}
      <div className="absolute inset-0 bg-gradient-to-br from-primary to-secondary text-white p-6 flex flex-col justify-center items-center text-center transition-opacity duration-500 group-hover:opacity-0 rounded-2xl">
        <h3 className="text-2xl font-bold mb-3">{project.name}</h3>
        <p className="text-sm text-white/90 line-clamp-3">{project.description}</p>
        <div className="flex flex-wrap gap-2 justify-center mt-4">
          {project.tech.slice(0, 3).map((tech) => (
            <span key={tech} className="bg-white/20 px-2 py-1 rounded-md text-xs">
              {tech}
            </span>
          ))}
        </div>
